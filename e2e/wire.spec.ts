/**
 * FM-9 Wire View e2e: the stage-flow panel is synchronized to the timeline
...
const involved = (page: Page) => page.locator('[data-testid="wire-view"] [data-involved]');
const stage = (page: Page, id: string) => page.locator(`[data-testid="wire-view"] [data-stage="${id}"]`);
...
test("closed by default, opens on demand, follows the playhead", async ({ page }) => {
  ...
  const details = page.getByTestId("wire-view");
  ...
  await expect(page.getByTestId("wire-what")).toContainText(/S1\/S2\/S3/);
  ...
  await expect(page.getByTestId("wire-correlations")).toContainText("surfaceId");
});
...
test("repair and lifecycle events stay off the render stages", async ({ page }) => {
  ...
  await expect(involved(page)).toHaveCount(2);
  await expect(page.getByTestId("wire-what")).toContainText(/repair message/);

  await jumpTo(page, 19, "RUN_FINISHED");
  await expect(involved(page)).toHaveCount(2);
  await expect(page.getByTestId("wire-what")).toContainText(/lifecycle/);
});
